...
import LogisticAATable from "./LogisticAATable";
import GroupOffersTable, {
  GROUP_VIEWS,
  GroupViewToggle,
  useScheduledGroupView,
} from "./GroupOffersTable";
...
import NewsTickerEn from "./NewsTickerEn";
import RatesStrip from "./RatesStrip";
import DealsExecTable from "./DealsExecTable";
import ARListTable from "./ARListTable";
import CeoMessage from "./CeoMessage";

import MembersHistoryChart from "./MembersHistoryChart";
import GroupSalesBars from "./GroupSalesBars";
import EventSlideshow from "./EventSlideshow";
import TotalDealsIcon from "./TotalDealsIcon";
import YearToDateTripsIcon from "./YearToDateTripsIcon";
import TvModeFrame from "./TvModeFrame";
import { DashboardNotice, DashboardSkeleton } from "./DashboardState";
...
// ---------- GroupDashboard ----------
export default function GroupDashboard({ groupKey: sharedGroupKey, isPublic = false }) {
  ...
  const { data: eventsData } = useSWR(isPublic ? null : "/api/events", fetcher, {
    revalidateOnFocus: false,
    refreshInterval: 5 * 60_000, // هر ۵ دقیقه یک‌بار چک کنه
  });

  const eventFiles = isPublic ? [] : ensureArray(eventsData?.files);
  ...
  const groupKey = sharedGroupKey || { 1: "A", 2: "B", 3: "C" }[id] || "A";
  ...
  if (!isReady || isLoading || !raw) {
    return (
      <DashboardSkeleton label={`Loading Group ${groupKey} dashboard…`} cards={8} panels={3} />
    );
  }
  ...
  const group =
    (!isPublic && groups.find((g) => toStr(g.id) === id)) ||
  ...
  const salesBarData = (isPublic ? [groupKey] : ["A", "B", "C"]).map((gKey) => {
