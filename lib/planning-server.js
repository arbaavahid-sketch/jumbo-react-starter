...
export async function loadPlanning({
  ...
  // Local development without Google credentials: read a workbook from disk instead of Drive.
  if (env.PLANNING_LOCAL_FILE && !sa) {
    if (!force && cache && Date.now() - cache.readAt < CACHE_MS) return { config, ...cache };
    const { readFileSync } = await import("node:fs");
    const { basename } = await import("node:path");
    const workbook = XLSX.read(readFileSync(env.PLANNING_LOCAL_FILE), { type: "buffer" });
    const sheets = workbook.SheetNames.map((name) => ({
      name,
      rows: XLSX.utils.sheet_to_json(workbook.Sheets[name], { header: 1, defval: "", raw: false }),
    }));
    cache = {
      data: parsePlanningWorkbook(sheets, config),
      source: {
        id: "local",
        name: basename(env.PLANNING_LOCAL_FILE),
        url: "",
        modifiedTime: null,
