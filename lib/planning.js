...
export function parseSalesPlan(rawRows) {
  ...
  if (control) {
    ...
    const col = (label) => header.findIndex((c) => normalizeName(c) === label);
    const qCols = ["q1", "q2", "q3", "q4"].map(col);
    const actualCol = col("actual");
    const remainingCol = col("remaining");
    const money = (r, c) => (c >= 0 ? parseMoney(cell(r, c)) : null);
    const line = (r) => ({
      target: parseMoney(cell(r, control.c + 1)),
    ...
    for (; r < rows.length; r += 1) {
      ...
      if (!name) continue;
      noteCurrency(cell(r, control.c + 1));
      if (normalizeName(name) === "total") {
        plan.groupsTotal = line(r);
        r += 1;
        break;
      }
      const group = name.match(/^tot\w*\s+(group\s+\w+)/i);
      if (group) {
        plan.groups.push({ name: group[1].replace(/^g/, "G"), people, total: line(r) });
        people = [];
        continue;
      }
    ...
    for (; r < rows.length; r += 1) {
      ...
      if (name && amount !== null) plan.extra.push({ name, amount });
    }
  }
