...
// Stable identity for one deal within a stage. The countdown for a deal is
// anchored to the first time this key was seen, so editing the sheet weekly
// only restarts the deals whose cell content actually changed. Whitespace and
// letter case are normalized so trivial edits do not reset the timer.
export function dealKey(stage, rawValue) {
  const norm = String(rawValue || "")
    .trim()
    .replace(/\s+/g, " ")
    .toLowerCase();
  return `${stage}::${norm}`;
