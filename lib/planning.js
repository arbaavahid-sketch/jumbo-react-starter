...
export function planningDigest(config, data, now = new Date()) {
  ...
  // Email HTML: tables and inline styles only, so it survives Outlook and Gmail.
  const shell = (body) =>
    `<div style="margin:0;padding:24px 12px;background:#f1f5fa;font-family:'Segoe UI',Tahoma,Arial,sans-serif;color:#142033">` +
    `<table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="width:100%;max-width:760px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden">` +
    `<tr><td style="padding:24px 28px;background:#173f7a;color:#ffffff">` +
    `<div style="font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#b7d9f8">Team planning</div>` +
    `<div style="font-size:24px;font-weight:700;padding-top:4px">Artin Azma planning</div>` +
    `<div style="font-size:14px;color:#d4e4f7;padding-top:2px">${escape(dateLabel)}</div>` +
    `</td></tr><tr><td style="padding:24px 28px">${body}</td></tr>` +
    `<tr><td style="padding:16px 28px 24px;border-top:1px solid #eef2f7;color:#94a3b8;font-size:12px">` +
    `Sent automatically every morning from the planning workbook.</td></tr></table></div>`;
  if (!data) {
    ...
    return {
      ...
      text: lines.join("\n"),
      html: shell(
        `<p style="margin:0;color:#526579">The planning workbook has not been read yet.</p>`,
      ),
    };
  }
  const body = [];
  ...
  lines.push(summary);
  const tile = (label, value, colour, soft) =>
    `<td width="25%" style="padding:0 6px" valign="top"><table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:100%;background:${soft};border-radius:12px"><tr><td style="padding:12px 14px">` +
    `<div style="font-size:12px;color:#526579">${escape(label)}</div>` +
    `<div style="font-size:26px;font-weight:700;color:${colour};line-height:1.2">${value}</div>` +
    `</td></tr></table></td>`;
  body.push(
    `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:100%;margin:0 0 18px"><tr>` +
      tile("Carried over", counts.carried, "#b45335", "#fdeee8") +
      tile("In progress", counts.open, "#173f7a", "#eef4fc") +
      tile("No status", counts.unstarted, "#96690b", "#fdf4e0") +
      tile("Closed", counts.done, "#176b48", "#e7f6ef") +
      `</tr></table>`,
    `<p style="margin:0 0 18px;color:#526579;font-size:13px">${escape(`${daysLeft} day${daysLeft === 1 ? "" : "s"} left in ${monthName}.`)}</p>`,
  );
  const milestones = data.milestones.filter((m) => m.month === period.month);
  if (milestones.length) {
    lines.push("", `${monthName} milestones`);
    for (const m of milestones) lines.push(`- ${m.title}${m.status ? ` [${m.status}]` : ""}`);
    body.push(
      `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:100%;background:#f3f8ff;border:1px solid #d5e5f7;border-radius:12px;margin:0 0 18px"><tr><td style="padding:14px 18px">` +
        `<div style="font-size:15px;font-weight:600;color:#173f7a;padding-bottom:6px">${escape(monthName)} milestones</div>` +
        `<ul style="margin:0;padding-left:20px;font-size:13px;color:#142033">` +
        milestones
          .map(
            (m) =>
              `<li style="padding:2px 0">${escape(m.title)}${m.status ? ` <em style="color:#64748b;font-style:normal">[${escape(m.status)}]</em>` : ""}</li>`,
          )
          .join("") +
        `</ul></td></tr></table>`,
    );
  ...
  if (plan) {
    ...
    if (month) {
      const reported = month.actual === null || month.actual === 0;
      const line = `${monthName} sales target: ${formatMoney(month.planned, plan.currency)} · actual: ${reported ? "not reported yet" : formatMoney(month.actual, plan.currency)}`;
      lines.push("", line);
      body.push(
        `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:100%;border:1px solid #e2e8f0;border-radius:12px;margin:0 0 18px"><tr>` +
          `<td style="padding:14px 18px"><div style="font-size:12px;color:#526579">${escape(monthName)} sales target</div>` +
          `<div style="font-size:22px;font-weight:700;color:#173f7a">${escape(formatMoney(month.planned, plan.currency))}</div></td>` +
          `<td style="padding:14px 18px;text-align:right"><div style="font-size:12px;color:#526579">Actual</div>` +
          `<div style="font-size:22px;font-weight:700;color:${reported ? "#94a3b8" : "#176b48"}">${escape(reported ? "not reported" : formatMoney(month.actual, plan.currency))}</div></td>` +
          `</tr></table>`,
      );
    }
    if (plan.priorities.length) {
      ...
      body.push(
        `<div style="font-size:15px;font-weight:600;color:#173f7a;margin:0 0 6px">Monthly priorities</div>` +
          `<ul style="margin:0 0 18px;padding-left:20px;font-size:13px;color:#142033">` +
          plan.priorities.map((p) => `<li>${escape(p)}</li>`).join("") +
          `</ul>`,
      );
    }
  }
  ...
  for (const group of planningGroups(tasks, config.teams || [])) {
    if (group.name) {
      ...
      body.push(
        `<div style="margin:22px 0 10px;padding-bottom:6px;border-bottom:2px solid #173f7a">` +
          `<span style="font-size:17px;font-weight:700;color:#173f7a">${escape(group.name)}</span>` +
          `<span style="font-size:13px;color:#526579"> — ${total} open task${total === 1 ? "" : "s"}</span></div>`,
      );
      if (!group.people.length) {
        lines.push("No open tasks.");
        body.push(`<p style="margin:0 0 12px;color:#526579;font-size:13px">No open tasks.</p>`);
      }
    }
    for (const person of group.people) {
      const tone = tones[personIndex % tones.length];
      personIndex += 1;
      ...
      const rows = person.tasks
        .map((task) => {
          ...
          const badge = task.status
            ? `<span style="display:inline-block;padding:2px 9px;border-radius:999px;background:#e8f1fd;color:#1d4f9c;font-size:11px;white-space:nowrap">${escape(task.status)}</span>`
            : `<span style="display:inline-block;padding:2px 9px;border-radius:999px;background:#fdf4e0;color:#96690b;font-size:11px;white-space:nowrap">no status</span>`;
          return (
            `<tr><td style="padding:8px 10px 8px 0;border-bottom:1px solid #eef2f7;font-size:13px" valign="top">` +
            `${escape(title)}` +
            (bucket === "carried"
              ? `<div style="color:#b45335;font-size:11px;padding-top:3px">${escape(flag)}</div>`
              : "") +
            `</td>` +
            `<td width="130" style="padding:8px 10px;border-bottom:1px solid #eef2f7" valign="top">${badge}</td>` +
            `<td width="38%" style="padding:8px 0 8px 10px;border-bottom:1px solid #eef2f7;font-size:12px;color:#526579" valign="top">${escape(comment)}</td></tr>`
          );
        })
        .join("");
      body.push(
        `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:100%;margin:0 0 14px">` +
          `<tr><td colspan="3" style="padding:10px 0 6px">` +
          `<table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>` +
          `<td width="26" style="width:26px;height:26px;background:${tone};border-radius:13px;color:#ffffff;font-size:11px;font-weight:700;text-align:center;vertical-align:middle">${escape(initialsOf(person.name))}</td>` +
          `<td style="padding-left:10px;font-size:14px;font-weight:600;color:#142033">${escape(person.name)}` +
          `<span style="font-weight:400;color:#526579"> — ${person.tasks.length} open</span></td>` +
          `</tr></table></td></tr>${rows}</table>`,
      );
    }
  }
  return { subject, text: lines.join("\n"), html: shell(body.join("")) };
}
